...
function setup() {
   ...
   for (let i = 0; i < startingFood; i++) {
      foods.push(createVector(random(screenEdge, width - screenEdge), random(screenEdge, height - screenEdge)));
   }
...
function draw() {
   ...
   for (let i = slimes.length - 1; i >= 0; i--) {
      ...
      if (isDebug) {
         ...
         stroke(0, 255, 0);
         strokeWeight(2);
         ellipse(slimes[i].position.x, slimes[i].position.y, slimes[i].dna.foodPerception, slimes[i].dna.foodPerception);
         noFill()
         stroke(255, 0, 0)
         ellipse(slimes[i].position.x, slimes[i].position.y, slimes[i].dna.dangerPerception, slimes[i].dna.dangerPerception);
      }
   ...
   for (let i = predators.length - 1; i >= 0; i--) {
      ...
      if (isDebug) {
         noFill();
         stroke(136, 29, 242);
         ellipse(predators[i].position.x, predators[i].position.y, predators[i].dna.killPerception, predators[i].dna.killPerception);
      }
